...
export default function PlanPayPage({ planName = 'Pro', closePage, openPage }) {
  ...
  return (
    <div className="po-body" style={{ maxWidth: 700 }}>
      {/* Plan summary */}
      <div className="plan-pay-summary">
        <div>
          <div className="pps-badge">선택하신 요금제</div>
          <div className="pps-name">{plan.name}</div>
          <div className="pps-price">₩{plan.price}<small>/월</small></div>
        </div>
        <div className="pps-features" dangerouslySetInnerHTML={{ __html: plan.features.replace(/\n/g,'<br/>') }}/>
      </div>
          ...
          <div className="pg-card" style={{ marginBottom: 16 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, fontSize: 14 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: 700, fontSize: 16 }}>
                <span>최종 결제금액</span><span style={{ color: 'var(--orange)' }}>₩{plan.total}</span>
              </div>
            </div>
      ...
      {success && (
        <div className="pp-success show">
          ...
          <div className="pp-action-row">
            ...
            <button className="pg-btn primary" onClick={() => openPage('mypage')}>마이페이지 →</button>
          </div>
        </div>
      )}
    </div>
